...
export const Indice = () => {
  ...
  useEffect(() => {( async () => {
    // Obtener la ruta en el navegador y si los dos primeros slug son diferentes de "user/garantias" entonces colocar el useRef de inicioRuta en "No"
  ...
  const obtenerGarantias = async (page = 1, per_page = 20, desde = 0) => {
    try {
      ...
      let queryParams = `?usuarioAdministrador=${usuarioAdministrador.current}&agenciaId=${agenciaId.current}&agenteId=${agenteId.current}&rolUsuarioConectado=${rolUsuarioConectado.current}&page=${page}&per_page=${per_page}&inicio_ruta=${inicioRuta.current}`;
