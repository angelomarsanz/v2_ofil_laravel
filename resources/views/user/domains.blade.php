        ...
        <div class="col-md-12">
                            ...
                            <form action="{{ route('user-domain-request') }}" id="customDomainRequestForm" method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <p class="text-secondary mb-0"><i class="fas fa-exclamation-circle"></i>
                                        {{ __('Do not use') }}
                                        <strong class="text-danger">http://</strong> {{ __('or') }} <strong
                                            class="text-danger">https://</strong>
                                    </p>
                                    <p class="text-secondary mb-0"><i class="fas fa-exclamation-circle"></i>
                                        {{ __('The valid format will be exactly like this one') }}
                                        - <strong class="text-danger">{{ __('domain.tld, www.domain.tld') }}</strong>
                                        {{ __('or') }}
                                        <strong class="text-danger">
                                            {{ __(' subdomain.domain.tld, www.subdomain.domain.tld') }}</strong></strong>
                                    </p>
                                </div>
                            </form>
                        </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="card-title d-inline-block">{{ __('Custom Domain') }}
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12">
                            @if (empty($rcDomain))
                                <h3 class="text-center">
                                    {{ __('REQUESTED OR CONNECTED CUSTOM DOMAIN NOT AVAILABLE') }}
                                </h3>
                            @else
                                <div class="table-responsive">
                                    <table class="table table-striped mt-3">
                                        <thead>
                                            <tr>
                                                <th scope="col">
                                                    {{ __('Requested Domain') }}</th>
                                                <th scope="col">
                                                    {{ __('Current Domain') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
