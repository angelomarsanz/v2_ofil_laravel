    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Add Area') }}</h5>
            ...
            <div class="modal-body">
                <form id="ajaxForm" class="modal-form create"
                    action="{{ route('admin.property_management.store_city') }}" method="post"
                    ...
                    @if ($bs->property_country_status == 1 && $bs->property_state_status == 1)
                        <div class="form-group" id="state">
                            <label for="">{{ __('City') }} <span
                                    class="text-danger">{{ '*' }}</span></label>
                    ...
                    @elseif($bs->property_country_status == 0 && $bs->property_state_status == 1)
                        <div class="form-group">
                            <label for="">{{ __('City') }} <span
                                    class="text-danger">{{ '*' }}</span></label>
                            <select name="state" class="form-control">
                    ...
                    @foreach ($Languages as $lang)
                        <div class="form-group {{ $lang->direction == 1 ? 'rtl text-right' : '' }}">
                            ...
                            <input type="text" class="form-control" name="{{ $lang->code }}_name"
                                placeholder="{{ __('Enter area name for') . ' ' . $lang->name . ' ' . __('language') }}">
