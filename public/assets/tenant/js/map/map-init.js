...
function initMap() {
    ...
    if (!google?.maps || !document.getElementById('map')) {
        document.getElementById('map').innerHTML = `
            <div style="height:100%;display:flex;flex-direction:column;justify-content:center;align-items:center;">
                <p style="color:#e74c3c;">Map Loading Failed</p>
                <button onclick="location.reload()" style="padding:6px 12px;background:#3498db;color:#fff;border:none;border-radius:4px;">আবার লোড করুন</button>
            </div>`;
        return;
    }
    ...
    map = new google.maps.Map(document.getElementById('map'), {
        center: {
            lat: -34.89635884201744,
            lng:  -56.166133885114576
        },
        zoom: 12
    });
    ...
    var input = document.getElementsByClassName('search-address')[0];
    ...
    google.maps.event.addListener(map, 'click', function (event) {
        ...
        setLatLng(latitude, longitude)
        geocodeLatLng(geocoder, map, clickedLocation, 'click_by');
    });
}

function geocodeLatLng(geocoder, map, latLng, type = null) {
