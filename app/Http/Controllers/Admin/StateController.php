...
{
    //tenantId == 0 means admin
    use AdminLanguage;
    public function index(Request $request)
    {
        $language = $this->currentLang();
        $name = request()->filled('name') ? request('name') : null;
        $information['tenantFrontLangs'] = $this->allLangs();
        $information['Languages'] = $this->allLangs();
        $states = State::getStates(0, $language->id, $name);

        $information['countries'] = Country::getCountries(0, $language->id);

        $information['states'] = collectionToPaginate($states, 10);
        return view('admin.property-management.state.index', $information);
    }
    public function getState(Request $request)
    {
        $language = $this->currentLang();
        $states = State::where([['user_id', 0], ['country_id', $request->id]])->get()->map(function ($state) use ($language) {
    ...
    public function store(Request $request)
    {
        ...
        // $basicSettings = BasicSetting::where('user_id', $tenantId)
